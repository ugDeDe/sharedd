...
// Терминальное завершение ноды (V7.9.11).
//
...
//	         снаружи. Путей назад ДВА (V7.9.12): перезапуск службы ПОСЛЕ
//	         СМЕНЫ ip (tombstone стирается на старте), либо перезапуск С
//	         ТЕМ ЖЕ ip — тогда запрашиваем у регистратора GP-перепроверку:
//	         регистратор V7.9.12+ возвращает такую ноду в карантин с одной
...
func checkTerminationTombstone(cfg *NodeConfig, ipr *ipResolver, client *http.Client) {
	...
	case reasonIPBan:
		...
		// Путь 2 (V7.9.12): ip тот же — просим у регистратора GP-перепроверку
		// вместо мгновенной смерти. Регистратор V7.9.12+ вернёт 200/429
