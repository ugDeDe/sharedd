...
| 📊 **Публичные страницы** | `/statistics` — обезличенная статистика каждой ноды для пользователей; `/dashboard` — история блокировок из бессрочной SQLite-базы |
...
Три страницы отдаёт сам бинарник регистратора (никаких внешних зависимостей —
HTML+JS вшиты, данные приходят по JSON API, автообновление каждые 5 с).
...
- **KPI-стрип**: живые назначения мастеров (N/M), нод в пуле, fully-healthy,
  размер очереди, активные клиенты (уникальные IP по метрикам telemt), кворум
  TCP/GP/Metrics-OK;
...
`dns_deleted`, `config_changed`, старт регистратора. Фильтр по типу и тексту.
...
#### 6️⃣ Настройки (hot-apply, без рестарта)
...
| 👤 **Пользователи MTProto** | Пары `username = secret` (32 hex); на нодах применяется add-only патчем `[access.users]` |
...
пишет `registry.toml`, `Caddyfile` и systemd-юниты, генерирует секрет первого
...
```bash
curl -fsSL https://raw.githubusercontent.com/ugDeDe/sharedd/main/scripts/install_node_web.sh | sudo bash
```
...
| `[panel] token` | — | Bearer для панели и `/status` (пусто = dev-режим) |
| `[cloudflare] domains` | — | Managed-домены: у каждого свой мастер |

`node.toml` на агенте — три строки; всё остальное нода получает с регистратора:
...
`[access.users]` — чужие записи никогда не перезаписываются). Удаление —
...
уникальные активные клиентские IP. Если нода не присылает per-user метрики
