...
// Жёсткое завершение нод (V7.9.11) — заменяет бесконечные циклы
...
//
//  1. ip_ban — нода не прошла globalping (V7.9.12: БЕЗ предусловий по
...
//     карантина — всё пишется в bans как ip_ban (V7.9.12).
//
//     Путей назад ДВА (V7.9.12): регистрация с НОВОГО ip снимает запись
...
//     «прокси был выключен». Бан в истории БД остаётся в обоих случаях.
//
...
//     Бессрочен. Исключение (V7.9.12): нода, умершая по tcp+metrics ВО
//     ВРЕМЯ GP-карантина, записывается как ip_ban (её класс уже определён
//     карантином); dead — только для нод, GP не фейливших.
//
// Транзиентное (smaller окна) поведение версий V7.9.6–V7.9.9 СОХРАНЕНО:
...
//
// Учёт в статистике (V7.9.14): строка в bans (публичный /dashboard, история
...
type TerminatedRecord struct {
	...
	// ReverifyFailed (V7.9.12) — нода уже получала перепроверку старого ip
	...
	// StaleIP (V7.9.13) — запись поставлена автоматически при выходе ноды
...
type QuarantineState struct {
	...
	// Reverify (V7.9.12) — карантин посажен переподключением СТАРОГО
...
// в статистику (V7.9.14): бан ноды, не задевшей пользователей, счётчики
// не двигает.
...
func (r *Registry) terminateNodeLocked(c *Candidate, now time.Time, reason, cause string) {
	...
	// V7.9.14: в историю — только бан ноды со временем мастерства. Ре-бан
	// по итогам reverify-карантина старого ip сюда не попадает (свежий
	// кандидат мастером не был): бан адреса уже учтён первой записью.
...
// В историю банов НЕ пишем (V7.9.14): кандидата нет — время мастерства
...
// (V7.9.12): переустановка агента генерирует новый id, но переподключение
...
// applyReverifyLocked — V7.9.12: старый забаненный ip переподключился тем
...
// quarantineIPChangeLocked — V7.9.13: нода сменила ip, будучи в карантине.
...
// строка bans(ip_ban) в вечной истории (п.3/п.5 ревью; V7.9.14 — только
// если у ноды было время мастерства) + заградительная stale-запись по
// СТАРОМУ ip (кто бы ни пришёл с него — reverify-трек V7.9.12). Сама
