...
// Персистентная история (V7.9.11): SQLite рядом с JSON-стейтом.
//
...
//	          V7.9.14: пишутся только баны нод, успевших поработать мастером
//	          (terminate.go hadMasterTime) — иначе reverify-перепроверки
//	          уже забаненных адресов накручивали бы счётчики.
...
// recordBan — терминальная блокировка ноды. gap считаем от предыдущего бана
// любого типа (периодичность «между событиями блокировки любых нод»).
