...
# fail_threshold подряд неудач гасит ноду (TCP-пробы и, с V7.9.4, metrics-отчёты
...
# V7.9.6: нода, непрерывно нездоровая дольше стольких минут, удаляется из пула
# (явный 0 = выключить рипер; ключ не задан = 60). V7.9.7: после prune нода ещё
...
# Принудительная ротация мастерства (V7.9.3): максимум непрерывных минут ноды
